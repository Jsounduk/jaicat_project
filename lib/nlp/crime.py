...
class CrimeAnalyzer:
    ...
    def load_crime_data(self):
        try:
            ...
            print("[CrimeAnalyzer] ✅ Crime data loaded successfully.")
            return data
        except FileNotFoundError:
            print(f"[CrimeAnalyzer] ❌ File not found at: {self.crime_data_path}")
            return None
        except Exception as e:
            print(f"[CrimeAnalyzer] ❌ Error loading crime data: {e}")
            return None
    ...
    def get_summary(self):
        if self.crime_data is not None:
            ...
            return "[CrimeAnalyzer] ⚠️ No crime data to summarize."
    def get_crime_types(self):
        if self.crime_data is not None:
            return self.crime_data['Crime Type'].unique()
        else:
            return "[CrimeAnalyzer] ⚠️ No crime data to analyze."
    def get_crime_by_type(self, crime_type):
        if self.crime_data is not None:
            ...
            if not filtered_data.empty:
                ...
                return f"[CrimeAnalyzer] ⚠️ No data found for crime type: {crime_type}"
        else:
            return "[CrimeAnalyzer] ⚠️ No crime data to filter."
    ...
    def get_crime_trends(self, time_column='Date'):
        if self.crime_data is not None and time_column in self.crime_data.columns:
            ...
            return "[CrimeAnalyzer] ⚠️ No crime data or invalid time column."
    def get_crime_heatmap(self, lat_column='Latitude', lon_column='Longitude'):
        if self.crime_data is not None and lat_column in self.crime_data.columns and lon_column in self.crime_data.columns:
            ...
            return "[CrimeAnalyzer] ⚠️ No crime data or invalid latitude/longitude columns."
    def save_crime_data(self, output_path):
        if self.crime_data is not None:
            try:
                self.crime_data.to_csv(output_path, index=False)
                print(f"[CrimeAnalyzer] ✅ Crime data saved to {output_path}")
            except Exception as e:
                print(f"[CrimeAnalyzer] ❌ Error saving crime data: {e}")
        else:
            print("[CrimeAnalyzer] ⚠️ No crime data to save.")
    def __str__(self):
