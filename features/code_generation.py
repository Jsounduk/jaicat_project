...
FLIRTY_TONES = [
    "Ohhh you want me to write *that*, huh? 😏",
    "Let’s make it dirty and delightful, babe…",
    "Here’s something that’ll make them blush 💋"
]

PROFESSIONAL_TONES = [
    "Here's a clean, professional version:",
    "This should work well for formal use.",
    "Crafted for a clear and polished tone:"
]

def generate_content(prompt: str, tone="default"):
    intro = ""
    if tone == "flirty":
        intro = random.choice(FLIRTY_TONES)
    elif tone == "professional":
        intro = random.choice(PROFESSIONAL_TONES)
    else:
        intro = "Got it. Here's something based on your prompt:"

    # Naive generation example
    body = f"\n\nBased on your prompt: '{prompt}', here's a suggested piece:\n"
    body += f"→ {prompt.capitalize()} is very important because..."
